...
#    ifdef SPI_SS_PIN
#        define BLUEFRUIT_LE_CS_PIN SPI_SS_PIN
#        pragma message "`BLUEFRUIT_LE_CS_PIN` is set as `SPI_SS_PIN`"
#    else
#        error "`BLUEFRUIT_LE_CS_PIN` is not set"
#    endif
#endif

#ifndef BLUEFRUIT_LE_SCK_DIVISOR
#    error "`BLUEFRUIT_LE_SCK_DIVISOR` is not set"
#    pragma message "Divide the MCU's clock speed by the desired 4MHz SPI clock speed"
#endif

#ifndef BLUEFRUIT_LE_IRQ_PIN
#    define BLUEFRUIT_LE_IRQ_PIN NO_PIN
#    pragma message "`BLUEFRUIT_LE_IRQ_PIN` is not set"
#endif

#ifndef BLUEFRUIT_LE_RST_PIN
#    define BLUEFRUIT_LE_RST_PIN NO_PIN
#    pragma message "`BLUEFRUIT_LE_RST_PIN` is not set"
#endif
...
#    define __(s)                          \
        do {                               \
            if (debug_bluetooth) print(s); \
        } while (false)
#    define __f(s, ...)                                  \
        do {                                             \
            if (debug_bluetooth) printf(s, __VA_ARGS__); \
        } while (false)
#else
// do nothing
#    define __(s)
...
#define BLE_CONNECTION_UPDATE_INTERVAL_MS 1000 /* milliseconds */

#define EVENT_FLAG_PROBED 0x01
#define EVENT_FLAG_USING  0x02
...
enum queue_type {
    QUEUE_TYPE_KEYBOARD, // 1-byte modifier + 6-byte key report
    QUEUE_TYPE_CONSUMER,  // 16-bit key code
    QUEUE_TYPE_MOUSE, // 4-byte mouse report
};
...
enum sdep_type_t {
    ...
    SPI_IGNORED_BYTE      = 0xFEu, // SPI default character. Character clocked out in case of an ignored transaction. Try again later.
    SPI_OVERREAD_BYTE     = 0xFF, // Character clocked out after an over-read of the transmit buffer. You read more data than is available.
};
...
enum ble_system_event_mask_t {
    BLE_SYSTEM_CONNECTED    = 0,
    BLE_SYSTEM_DISCONNECTED = 1,
    // BleSystemUartRx         = 8,
    // BleSystemMidiRx         = 10,
};
...
#define BLE_DEFAULT_TIMEOUT_MS 150
#define BLE_SHORT_TIMEOUT_MS 10
#define SPI_DEFAULT_DELAY_US 50

static bool at_command(const char *cmd, char *resp, uint16_t resplen, bool verbose, uint16_t timeout = BLE_DEFAULT_TIMEOUT_MS);
...
static bool sdep_send_pkt(const struct sdep_msg *msg, uint16_t timeout) {
    ...
    do {
        ready = spi_write(msg->type) != SPI_IGNORED_BYTE;
        if (ready) {
            break;
        }
...
static inline void sdep_build_pkt(struct sdep_msg *msg, uint16_t command, const uint8_t *payload, uint8_t len, bool moredata) {
    msg->type     = SDEP_MSGTYPE_COMMAND;
    msg->cmd_low  = command & 0xFF;
    msg->cmd_high = command >> 8;
    msg->len      = len;
    msg->more     = (moredata && len == SDEP_MAX_PAYLOAD) ? 1 : 0;
...
static bool sdep_recv_pkt(struct sdep_msg *msg, uint16_t timeout) {
    ...
    if (ready) {
        ...
        do {
            ...
            msg->type = spi_read();
            if (msg->type == SPI_IGNORED_BYTE || msg->type == SPI_OVERREAD_BYTE) {
                // Release it and let it initialize
                spi_stop();
                wait_us(SPI_DEFAULT_DELAY_US);
                spi_start(BLUEFRUIT_LE_CS_PIN, false, 0, BLUEFRUIT_LE_SCK_DIVISOR);
...
static void resp_buf_read_one(bool greedy) {
    ...
    again:
        if (sdep_recv_pkt(&msg, BLE_DEFAULT_TIMEOUT_MS)) {
            if (!msg.more) {
                // We got it; consume this entry
                resp_buf.get(last_send);
                __f("recv latency %dms\n", TIMER_DIFF_16(timer_read(), last_send));
            }
    ...
    } else if (timer_elapsed(last_send) > BLE_DEFAULT_TIMEOUT_MS * 2) {
        __f("waiting_for_result: timeout, resp_buf size %d\n", (int)resp_buf.size());
...
static void send_buf_send_one(uint16_t timeout = BLE_DEFAULT_TIMEOUT_MS) {
    ...
    if (process_queue_item(&item, timeout)) {
        // commit that peek
        send_buf.get(item);
        __f("send_buf_send_one: have %d remaining\n", (int)send_buf.size());
    } else {
        __("failed to send, will retry\n");
        wait_ms(BLE_DEFAULT_TIMEOUT_MS);
...
void bluefruit_le_init(void) {
    ...
    wait_ms(500);  // wait 1/2 a sec for debugging
#endif
}
...
static bool read_response(char *resp, uint16_t resplen, bool verbose) {
    ...
    while (true) {
        ...
        if (!sdep_recv_pkt(&msg, 2 * BLE_DEFAULT_TIMEOUT_MS)) {
            __("sdep_recv_pkt failed\n");
            return false;
        }

        if (msg.type != SDEP_MSGTYPE_RESPONSE) {
            *resp = 0;
            return false;
        }
...
static bool at_command(const char *cmd, char *resp, uint16_t resplen, bool verbose, uint16_t timeout) {
    ...
    while (end - cmd > SDEP_MAX_PAYLOAD) {
        sdep_build_pkt(&msg, SDEP_CMDTYPE_AT_WRAPPER, (uint8_t *)cmd, SDEP_MAX_PAYLOAD, true);
    ...
    sdep_build_pkt(&msg, SDEP_CMDTYPE_AT_WRAPPER, (uint8_t *)cmd, end - cmd, false);
    if (!sdep_send_pkt(&msg, timeout)) {
        return false;
    }
...
bool bluefruit_le_initialize(void) {
    struct sdep_msg msg;
    ...
    sdep_build_pkt(&msg, SDEP_CMDTYPE_INITIALIZE, NULL, 0, false);
    state.initialized = sdep_send_pkt(&msg, BLE_DEFAULT_TIMEOUT_MS);

    if ((BLUEFRUIT_LE_RST_PIN != NO_PIN) && !state.initialized) {
        // Perform a hardware reset
        __("Performing a hardware reset\n");
...
void bluefruit_le_task(void) {
    ...
    if (!state.initialized && !bluefruit_le_initialize()) {
        return;
    }
    ...
    resp_buf_read_one(true);
    send_buf_send_one(BLE_SHORT_TIMEOUT_MS);

    if (resp_buf.empty() && (state.event_flags & EVENT_FLAG_USING) && readPin(BLUEFRUIT_LE_IRQ_PIN)) {
        // Must be an event update
        if (at_command_P(PSTR("AT+EVENTSTATUS"), resbuf, sizeof(resbuf))) {
            uint32_t mask = strtoul(resbuf, NULL, 16);

            if (mask & BLE_SYSTEM_CONNECTED) {
                set_connected(true);
            } else if (mask & BLE_SYSTEM_DISCONNECTED) {
                set_connected(false);
            }
        }
    }

    if (timer_elapsed(state.last_connection_update) > BLE_CONNECTION_UPDATE_INTERVAL_MS) {
        bool shouldPoll = true;
        if (!(state.event_flags & EVENT_FLAG_PROBED)) {
            ...
            if (at_command_P(PSTR("AT+EVENTENABLE=0x1"), resbuf, sizeof(resbuf))) {
                ...
                state.event_flags |= EVENT_FLAG_USING;
            }
            state.event_flags |= EVENT_FLAG_PROBED;
...
static bool process_queue_item(struct queue_item *item, uint16_t timeout) {
    ...
    if (TIMER_DIFF_16(state.last_connection_update, item->added) > 0) {
        __f("send latency %dms\n", TIMER_DIFF_16(state.last_connection_update, item->added));
    }
#endif

    switch (item->queue_type) {
        case QUEUE_TYPE_KEYBOARD:
...
void bluefruit_le_send_keyboard(report_keyboard_t *report) {
    ...
    item.queue_type   = QUEUE_TYPE_KEYBOARD;
...
void bluefruit_le_send_consumer(uint16_t usage) {
    ...
    item.queue_type = QUEUE_TYPE_CONSUMER;
    item.consumer   = usage;
...
void bluefruit_le_send_mouse(report_mouse_t *report) {
    ...
    item.queue_type        = QUEUE_TYPE_MOUSE;
